...
- Gate 事件合约通过已登录指纹浏览器页面被动读取 BTC 5m/15m 双向盘口；订单捕获模式会持续记录一次手动订单期间 Gate 域名下请求、响应和 WebSocket 的脱敏元数据，并可导出链路供确认最终接口，不保存 Cookie、签名、查询 token 或完整请求体。捕获验证后才允许显式开启 Gate↔Kalshi 人工双腿实盘；APIv4 Key 仍只用于官方只读余额接口，不把现货接口当作事件合约接口；
...
授权码绑定机器并使用系统安全存储加密保存。软件不再每15秒轮询：启动、系统唤醒、窗口重新聚焦和每次受保护操作前会顺手校验，并根据授权到期时间设置单次定时检查；另有5分钟一次的本地兜底检查，全程不联网。到期后停止自动开单并退出交易主界面。若当时存在已成交、未知成交或恢复中敞口，只显示应急恢复/平仓功能，处理完成后返回授权页。离线授权能阻止普通复制使用，但 Electron 客户端仍可被有能力的攻击者修改，因此它属于商业使用约束，不等同于不可破解的 DRM。

### 客户如何找到机器码

1. 第一次打开 ArbDesk 时，软件会直接显示授权页面，不会进入交易主界面。
2. 在页面中部找到“本机机器码”，点击右侧复制按钮。
3. 把以 `ARB-` 开头的完整机器码发给授权管理员；不要发送账户密码、助记词或交易私钥。
4. 收到以 `ARB1.` 开头的授权码后，粘贴到授权码输入框，点击“验证并进入软件”。
5. 已经进入软件但需要续期时，打开“右上角设置 → 账户与环境 → 软件授权”，可再次复制同一机器码并查看到期时间。

可直接转发给客户的简版说明见 [`docs/客户授权教程.md`](docs/客户授权教程.md)。
