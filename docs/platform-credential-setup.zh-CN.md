...
3. 由你本人在 Gate 页面完成一次最小金额订单。此动作不是程序自动提交；程序会持续记录该次操作期间 Gate 域名下的请求、响应和 WebSocket 帧的脱敏元数据，包括路径、方法、字段名、GraphQL operationName、状态码和订单号，不保存原始 body。
4. 手动完成后不要立刻关闭捕获模式，先观察链路计数（请求/响应/WS），点击 `导出脱敏订单链路供分析` 保存元数据；我会据此确认最终提交接口和成交回读链路。
5. 页面显示“已捕获订单结构”后，先点击 `完整联调 Gate（绝不下单）` 检查行情、余额和捕获字段，再按需开启 Gate 实盘开关。捕获模式持续到你主动停止或清除，不会因为第一条写请求就结束。
6. 导出的文件只包含脱敏元数据；Cookie、Authorization、签名、查询 token 和完整请求体不会写入文件。订单 POST 超时或状态不明只进入回读，不自动重发。
